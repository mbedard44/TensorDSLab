...
- source units, roles, coordinates, device, dtype, count/order, positional
  structural binding, and failure before allocation/RNG;
- Kernel conditioning-coordinate reorder, dimension permutation, unit/device
...
Spec by identity. Production and validation admit a structurally equal
replacement object but reject changed unit, semantic Spec, axis/coordinate
state, device, dtype, count, or order. Production performs this check before
source conversion, arithmetic, allocation, or RNG words.
...
Implementation runs focused and complete local source-form evidence. Validation
